...
which shows that this set \( \calP \) is the span of the columns of the matrix \( A \).  The span of the columns takes a particularily simple form if a similarity transformation takes the matrix to its Jordan canonical form.  This is a basis for which the representation of the linear transformation leaves the matrix representation of that transformation either completely diagonal, or in specific upper triangular form with zeros everywhere but the superdiagonal (and only ones or zeros on the superdiagonal).  Such a similarity transformation
...
\begin{dmath}\label{eqn:ProblemSet1Problem6:n}
\begin{bmatrix}
3 & 1 \\
-1 & 1
\end{bmatrix}
...
for which the oriented area given by the determinant is clearly unity in both cases.
