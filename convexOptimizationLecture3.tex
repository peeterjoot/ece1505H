...
\beginArtNoToc
\generatetitle{ECE1505H Convex Optimization.  Lecture 3: XXX.  Taught by Prof.\ Stark Draper}
%\chapter{XXX}
...
\begin{dmath}\label{eqn:convexOptimizationLecture3:n}
\Bx_2 + \theta (\Bx_1 - \Bx_2),
\end{dmath}
...
\begin{dmath}\label{eqn:convexOptimizationLecture3:n}
\conv(C) = \setlr{ \sum \theta_i \Bx_i | \Bx_i \in C, \theta_i \ge 0, \sum \theta_i = 1 }.
\end{dmath}
} % definition
...
A set C is a cond if \forall x in C and \forall \theta \ge 0 we have \theta \Bx \in C.

Scale uot if \theta > 1.  Scale in if \theta < 1.

F6

A convex cond is a cone that is also a convex set.

Conic combination

\sum \theta_i x_i, \theta_i \ge 0
