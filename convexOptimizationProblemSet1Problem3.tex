...
Verify that \( S^n \subseteq \Rm{n \times n} \) is a vector space under the regular matrix addition and scaling (multiplication by scalars in \(\bbR\)) operations.
...
Verify that \( \innerproduct{A}{B} = \Tr(A^\T B) \) where \( A, B \in S^n \) satisfies all the properties of an inner product.
} % makeproblem
